        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div v-if="mode === 'register'">
            ...
            <input 
              v-model="inviteCode"
              type="text" 
              placeholder="若有教师邀请码，请输入以获得教师权限"
              class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-orange-400 transition-colors"
              :disabled="loading"
            />
            <div v-if="showInviteTip" class="text-xs text-green-600 mt-1.5 font-bold animate-pulse flex items-center gap-1">
              <span>✨</span> 已从邀请链接自动填入注册码，注册后直接获得教师权限！
            </div>
          </div>
