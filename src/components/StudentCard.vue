...
import { getPetType, getLevelProgress, calculateLevel, getPetLevelImage } from '@/data/pets'
...
function handleClick() {
  if (studentStore.batchMode) {
    studentStore.toggleStudentSelect(props.student.id)
  } else if (studentStore.showDeleteStudentMode) {
    studentStore.toggleDeleteStudent(props.student.id)
  } else {
    // 由父组件处理详情面板打开
    emit('click')
  }
...
<template>
  <div
    ...
    :class="[getLevelBorderClass(getDisplayLevel(student)), {
      'ring-2 ring-purple-400 ring-offset-2': studentStore.batchMode && studentStore.selectedStudents.has(student.id),
      'ring-2 ring-red-400 ring-offset-2': studentStore.showDeleteStudentMode && studentStore.deleteStudentList.includes(student.id)
    }]"
    @click="handleClick"
  >
    <!-- 评分动效 -->
    <Transition name="score-pop">
      <div
        v-if="studentStore.scoreAnimations.has(student.id)"
        class="absolute inset-0 flex items-center justify-center z-20 pointer-events-none"
      >
        <div
          class="text-4xl font-bold animate-bounce-in"
          :class="studentStore.scoreAnimations.get(student.id)!.points > 0 ? 'text-green-500' : 'text-red-500'"
        >
          {{ studentStore.scoreAnimations.get(student.id)!.points > 0 ? '+' : '' }}{{ studentStore.scoreAnimations.get(student.id)!.points }}
        </div>
        <div class="absolute inset-0 overflow-hidden">
          <span v-for="i in 6" :key="i" class="absolute text-2xl animate-sparkle" :style="{ left: `${Math.random() * 80 + 10}%`, top: `${Math.random() * 80 + 10}%`, animationDelay: `${i * 100}ms` }">
            {{ studentStore.scoreAnimations.get(student.id)!.points > 0 ? '⭐' : '💫' }}
          </span>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="pop">
      <div
        v-if="studentStore.batchMode || studentStore.showDeleteStudentMode"
        class="absolute top-3 left-3 w-7 h-7 rounded-full flex items-center justify-center z-10 shadow-md transition-all"
        :class="studentStore.batchMode
          ? (studentStore.selectedStudents.has(student.id) ? 'bg-gradient-to-r from-purple-400 to-pink-400' : 'bg-white border-2 border-gray-300')
          : (studentStore.deleteStudentList.includes(student.id) ? 'bg-gradient-to-r from-red-400 to-pink-400' : 'bg-white border-2 border-gray-300')"
      >
        <span v-if="(studentStore.batchMode && studentStore.selectedStudents.has(student.id)) || (studentStore.showDeleteStudentMode && studentStore.deleteStudentList.includes(student.id))" class="text-white text-sm font-bold">✓</span>
      </div>
    </Transition>

    <!-- 设备心跳与电量状态栏 -->
    <div
      v-if="!studentStore.batchMode && !studentStore.showDeleteStudentMode && student.device_id"
    ...
    >
      ...
      <span v-if="student.is_charging" class="text-amber-500 flex items-center gap-0.5">⚡ {{ student.battery_level }}%</span>
      <span v-else>🔋 {{ student.battery_level ?? 100 }}%</span>
    </div>
    ...
    >
      <template v-if="student.pet_type">
        <div class="w-full h-full overflow-hidden" style="border-radius: 14px 14px 0 0; margin: -1px -1px 0 -1px; width: calc(100% + 2px);">
          <PetImage :src="getStudentPetImage(student)" :alt="getPetType(student.pet_type)?.name" size="full" :rounded="false" :show-loading="true" class="w-full h-full" />
        </div>
      </template>
