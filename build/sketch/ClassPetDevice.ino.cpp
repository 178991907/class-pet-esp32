#include <Arduino.h>
#line 1 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
/**
...
#line 196 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
void onPomodoroFinished();
#line 220 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
void setup();
#line 261 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
void loop();
#line 431 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
void syncOfflineQueue();
#line 460 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
void checkHardwareButtons();
#line 528 "/Users/Terry/Downloads/class-pet-main/firmware/ClassPetDevice/ClassPetDevice.ino"
    ...
    case STATE_NORMAL_ONLINE: {
      ...
      if (millis() - lastStatusCheck > STATUS_CHECK_INTERVAL) {
        ...
        bool isCharging = false;
          ...
          float batVoltage = (rawAdc / 4095.0) * 3.3 * 2.0;
          batteryPercent = (int)map(batVoltage * 100, 330, 420, 0, 100);
          if (batteryPercent < 0) batteryPercent = 0;
          if (batteryPercent > 100) batteryPercent = 100;
          isCharging = (batVoltage > 4.25);
        #endif
        ApiClient::sendHeartbeat(batteryPercent, isCharging);
        display->updateBatteryStatus(batteryPercent, isCharging);
        ...
        if (res.success && res.status == "ok") {
          ...
          if (!res.success) {
            DEBUG_PRINTLN("❌ 云端拉取失败，弹出中文报错警告");
            display->showChineseErrorScreen("API Server Error", res.error_msg);
            delay(4000); // 挂起展示 4 秒
          } else if (res.status == "unbound") {
            display->showChineseErrorScreen("Device Unbound", "Please bind student in admin dashboard");
            delay(4000);
          }
...
void checkHardwareButtons() {
  ...
  if (touch.getTouch(tx, ty)) {
    if (millis() - lastTouchTime > 500) { // 500ms 防连击抖动
      lastTouchTime = millis();
      DEBUG_PRINTF("👉 屏幕被触摸: X=%d, Y=%d\n", tx, ty);
      ...
      if (tx < 160) {
        if (tomatoTimer.isActive()) {
          ...
          display->showTextToast("🍅 番茄钟已终止并返回待机", 3000);
        } else {
      ...
      // 右半边屏幕：语音交互模拟
        ...
        if (systemState == STATE_NORMAL_ONLINE) {
          ...
          display->showTextToast("⚠️ 离线状态，语音交互不可用", 3000);
        }
      }
    }
  }
